...
test('Wave 3 product convergence: approved UI runs on the real engine through HTTP', async () => {
  ...
  try {
    ...
    const rowClasses = /<div class="([^"]*)" style="--i:\d+" data-trip-id="trip_a"/.exec(dashboardDuring.html)?.[1] ?? '';
    ...
    assert.ok(travellerDuring.html.includes('value="Approve"'), 'the Approve option button exists');
    assert.ok(travellerDuring.html.includes('value="Decline"'), 'the Decline option button exists');
    ...
    const approveCard = /<button[^>]*value="Approve"[^>]*>([\s\S]*?)<\/button>/.exec(travellerDuring.html);
