...
test('i5: HTTP surface serves real projections and the decision endpoint drives the loop', async () => {
  ...
  const server = createAppServer({ environment: 'local', logLevel: 'info', adapterMode: 'REPLAY', httpPort: 0, sqlitePath: ':memory:', recordingsDir: 'recordings', fixturesDir: 'fixtures', providers: { atlas: { env: 'sandbox' }, modelStudio: {}, googleRoutes: {}, nuitee: {} } }, endpoints);
