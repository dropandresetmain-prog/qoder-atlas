...
export async function persistRecoveryStrategy(
  ...
  try {
    strategy = RecoveryStrategySchema.parse(params.strategy);
  } catch (error) {
  ...
  if (!isNonEmptyManifest(strategy.baseManifest)) {
    return {
      ok: false,
      conflict: typedConflict('VALIDATION_FAILED', 'EMPTY_BASE_MANIFEST: RecoveryStrategy requires a non-empty base_manifest', [
        ref('RECOVERY_STRATEGY', strategy.id),
      ]),
    };
  }
  if (strategy.affectedSubjectRefs.length === 0) {
    return {
      ok: false,
      conflict: typedConflict('VALIDATION_FAILED', 'affected subjects required', [
        ref('RECOVERY_STRATEGY', strategy.id),
      ]),
    };
  }
  if (scenarioChange.recoveryStrategyId !== strategy.id) {
    return {
      ok: false,
      conflict: typedConflict('VALIDATION_FAILED', 'scenarioChange.recoveryStrategyId must match strategy.id', [
        ref('RECOVERY_STRATEGY', strategy.id),
      ]),
    };
  }
  if (scenarioChange.strategyVersion !== strategy.strategyVersion) {
    return {
      ok: false,
      conflict: typedConflict('VALIDATION_FAILED', 'scenarioChange.strategyVersion must match strategy.strategyVersion', [
        ref('RECOVERY_STRATEGY', strategy.id),
      ]),
    };
  }
