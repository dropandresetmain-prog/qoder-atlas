...
export const FORBIDDEN_UI_ENUM_PATTERN =
  /\b(?:PASS|FAIL|UNKNOWN|VIABLE|NOT_VIABLE|EVALUATED|PROPOSED|SELECTED|AUTHORIZED|EXECUTING|RECONCILING|COMPLETED|OUTCOME_UNKNOWN|OPEN|PLANNING|RESOLVED|SUPERSEDED|CANCELLED|CLOSED|AWAITING_AUTHORITY|REJECTED_DETERMINISTIC|REJECTED_VALIDATION|VIABLE_NOT_RECOMMENDED|RECOMMENDED|HEALTHY|AFFECTED|FAILED|CHANGED|RECOVERED|ACTIVE)\b/;
...
export const CASE_REASON_SENTENCE: Record<string, string> = {
  ...
  connection_too_short: 'there is not enough time to connect',
  connection_time_unknown: 'the connection time is not confirmed',
  credential_missing: 'a required travel credential is missing',
  ...
  active_original_stay_misaligned: 'the booked stay no longer covers the arrival date',
  arrival_aligned_replacement_missing: 'a date-aligned replacement stay is still required',
  original_stay_covers_arrival: 'the booked stay still covers the arrival date',
  original_stay_arrival_aligned: 'the booked stay starts on the arrival date',
  replacement_stay_arrival_aligned: 'the replacement stay matches the arrival date',
