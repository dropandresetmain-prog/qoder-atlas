...
import {
  CHANGE_LABEL, FOCUS_LABEL, SEMANTIC_GLYPHS, SEMANTIC_ICONS, TONE_CLASS, TRUTH_LABEL,
} from './grammar.ts';
...
export function semanticNode(node: PresentationNode): string {
  return `<article class="sem-node ${TONE_CLASS[node.indicator.tone]}" data-ref="${esc(node.ref)}" data-state="${esc(node.semanticState)}" data-kind="${esc(node.entityKind)}" data-truth="${node.truthMode}" data-change="${node.changeState}" data-focus="${node.focusRole}">
