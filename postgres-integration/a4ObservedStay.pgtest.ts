...
import { beginSeed, commitSeed, seedChildSubject, seedExternalConnection, seedJourney, seedOrganisation, seedPlace, seedTraveller, seedTrip, takeSeedEvidence } from './m3Seed.ts';
...
describe('A4 atomic observed stay attachment', () => {
  ...
  test('refuses a cancellation whose requested item is not allocated to the reservation, without partial writes', async () => {
    ...
    assert.deepEqual(state.rows, [{
      ...
      journey_revision: String(attached.journeyRevision),
      reservation_revision: String(attached.reservationRevision),
    }]);
  });

  test('optional canonicalApplication absent is valid; malformed supplied canonical IDs are rejected', async () => {
    const f = await setup();
    const uow = () => new PgUnitOfWork(f.pool, f.seed.workspaceId);
    const attached = mustOk(await attachObservedStay(uow(), input(f, randomUUID())));
    const base = {
    ...
    const malformed = await applyObservedStayCancellation(uow(), {
      ...base,
      idempotencyKey: randomUUID(),
    ...
    const without = mustOk(await applyObservedStayCancellation(uow(), {
      ...base,
      idempotencyKey: randomUUID(),
    }));
    assert.ok(without.journeyRevision > attached.journeyRevision);
  });
});
