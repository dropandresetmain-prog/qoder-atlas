...
- **Adapter-side ceiling re-check (G3R-R1 fix A3)**: `payOrder` no longer trusts the single executor-side ceiling check alone. In LIVE/RECORD only, it calls its own `retrieveOrder` before `/pay.do` and enforces `query.authorisedAmount` itself, allowlisted to an observed HELD order — a denylist of non-payable states was rejected because it falls through on an unmapped status that might already be paid. Observed `PAID`/`TICKETING`/`TICKETED` short-circuits to the observed state with no second `/pay.do` call; observed `UNKNOWN` or a failed status query refuses `atlas_payable_unverifiable`; any other non-HELD state refuses `atlas_order_not_payable`; missing payable total refuses `atlas_payable_unverifiable`; currency mismatch refuses `atlas_payable_currency_mismatch`; payable above ceiling refuses `atlas_payable_exceeds_authorised`. REPLAY is exempt (no provider call; the executor's `paymentGateVerdict` already ran against the same recorded create data). See ADR-047.
