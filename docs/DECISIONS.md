...
## ADR-051 — Application-owned booking dossiers wired into the composed executor (Mission 2 integration seam)
**Status:** Accepted (Wave 3R Mission 2, branch `wave3r/product-build`)

The frozen Traveller ontology carries a display name only — no structured provider booking identity (given/family split, date of birth, gender, contact). ADR-050 doctrine made provider-backed execution fail closed rather than guess identity, but the composed application wired no dossier at all, so REPLAY flows never traversed the provider-backed executor Mission 3 will run LIVE. This ADR closes that seam.

- **Application-owned store** (`src/app/dossierStore.ts`): an SQLite table `booking_dossiers` beside the frozen persistence schema (the `preferenceStore.ts` precedent), validated on every read/write. Bundle wire shape `booking-dossiers.json` per scenario directory, seeded through the SAME generic `seedScenarioBundle` path every other bundle fact uses, with referential integrity enforced (every dossier must name a traveller the bundle seeds).
- **Resolution is per-intent through the authoritative graph** (`src/app/dossierResolver.ts`): `intent.caseId → RecoveryCase.tripId → Trip.travellerIds → store`. Never scenario-keyed, never derived from LLM output. Hotel replacement takes the replacement rate id from intent `bookingRefs[0].reference` and the displaced stay from the trip's STAY element `bookingRef` — absent evidence ⇒ absent field, never invented.
- **Fail-closed semantics unchanged** (`src/app/providerExecution.ts`): absent dossier ⇒ REPLAY keeps the simulation fallback; LIVE/RECORD refuses with `booking_dossier_absent` / `hotel_dossier_absent`. Dossier deps are awaitable so resolution may read authoritative state.
- **Reset/reseed wipes and reseeds dossiers** (`src/app/runtime.ts` + `compose.ts`) through the same bundle path — deterministic reset with no residue.

REPLAY proof is decisive, not asserted: scenario-local `order_create`/`order_pay`/`order_retrieve` recordings are keyed by the deterministic recording id over (provider, operation, request) — and the request embeds the seeded dossier identity plus the composed intent id as `clientReference`. A REPLAY-provenance SUCCESS with providerId atlas therefore proves the executor booked with the resolved dossier; any other identity (or none ⇒ simulation fallback) misses every recording. Permanent tests: `test/wave3r-m2-dossier-wired-replay.test.ts` (seed/referential integrity; REPLAY traverses the provider-backed executor; wiped dossier ⇒ simulation fallback; reset reseeds). `wave3-case-b-hero-disruption.test.ts` updated: REPLAY now runs provider-backed execution, not simulation. Also: `DeterministicFallbackPlanner` now receives the deterministic id factory in composition (REV-2 WP-R5 doctrine extends to persisted strategy ids feeding intent ids).

Contract delta: none — no frozen schema changed; `ComposedRuntime` gains an additive `dossierStore` export for integration evidence and Mission-3 harnesses.

## ADR-052 — Generic FX / home-currency normalization for authority and planning (provider-neutral)
**Status:** Accepted (NORTHSTAR wave, branch `wave3r/fx-home-currency`)

Provider quotes arrive in arbitrary currency; organisation policy is stated in a configurable home currency. Authority previously compared spend only when currencies matched exactly (ADR-045 fail-closed), which blocked legitimate cross-currency recovery. This ADR adds deterministic, evidence-backed normalization WITHOUT weakening any fail-closed discipline.

- **Configurable home currency**: `Organisation.homeCurrency` (optional, ISO-4217 triplet). Resolved per trip through the governing rule-set owners (`resolveHomeCurrency`): alphabetical owner order is deterministic; disagreement between owners yields NO home currency (fail closed). Trips with no owning organisation stay unnormalized. No global default exists in engine logic — demo fixtures state SGD explicitly.
- **FX rate evidence** (`src/engine/fx.ts`): `FxRateEvidence` records carry base/home currency, a strictly positive quote-style rate, provenance (`sourceId`, fact-ladder `authority`) and an effective period (`observedAt`, optional `validUntil`). Staleness is judged against the comparison instant, never a wall clock. Only AUTHORITATIVE/CONNECTED evidence may support policy comparison; ASSERTED/INFERRED rates are untrusted and fail closed.
- **Deterministic conversion**: `convertMoney` rounds half-away-from-zero at a fixed 2-decimal scale into the TARGET currency; identical inputs always produce identical outputs. No LLM anywhere on this path; no live FX calls from deterministic code.
- **Single normalization seam**: `fxNormalizeSpend` restates a provider-currency gross spend into home currency or returns structured failure data (`no_rate_evidence` / `rate_expired` / `rate_untrusted`). Same-currency amounts pass through as exact identity without evidence. Every failure BLOCKs authority (ADR-045 preserved).
- **Authority evaluation order unchanged, comparisons generalized** (`src/engine/authority.ts`): SPEND_LIMIT / APPROVAL_ABOVE_SPEND compare against every deterministically-known statement of the charge (frozen original provider amount, its home restatement); when several statements are comparable, the LARGEST governs so a favourable FX move can never launder a breach past a ceiling. Incomparable currencies still fail closed exactly as before.
- **Intent freezing preserves executor ceilings**: when normalization succeeds at intent construction, `spendExposure` becomes the home restatement authority reviewed, and the ORIGINAL provider amount/currency is frozen verbatim as `ActionIntent.providerSpend` plus an audited `fxNormalization` record. The executor's payment ceiling is `providerSpend ?? spendExposure` — an FX-rate change after authorisation can never raise the already-authorised provider charge. A post-authority strategy mutation cannot either (the executor reads only intent fields).
- **Persistence + seeding**: application-owned SQLite store (`src/app/fxStore.ts`, table `fx_rates`) beside the frozen persistence schema; optional scenario-bundle file `fx-rates.json` seeds through the same generic `seedScenarioBundle` path as dossiers; runtime reset wipes/reseeds it identically. Evidence resolution is by currency pair + instant only — never scenario-keyed.
- **Read models/UI**: case views may expose BOTH the normalized home amount and the original provider charge (`RecoveryOptionView.providerCost`), keeping the two visibly distinct.

Contract delta: additive only, but reported explicitly per NORTHSTAR discipline — `Organisation.homeCurrency?`, `ActionIntent.providerSpend?` + `ActionIntent.fxNormalization?` (persisted record schema in `src/operational/fxRecord.ts`), `AuthorityContext.homeCurrency?` + `AuthorityContext.fxRates?`, new `FxRateResolver` service seam, `SeedOutcome.fxRateCount`. Permanent tests: `test/northstar-fx-normalization.test.ts` pins the full matrix including same-currency identity, valid/expired/missing/untrusted evidence, original-amount preservation, post-authority ceiling immutability, and an alternate-home-currency dataset (EUR/JPY) proving no SGD hardcoding.

**Supplement (same ADR): external reference-rate provider.** `FrankfurterFxAdapter` (`src/providers/frankfurter/adapter.ts`) adds a key-less external source — the public Frankfurter API (`https://api.frankfurter.dev/v1`, ECB-published official reference rates; SGD is among its supported currencies, so an SGD quote is genuine central-bank reference evidence). It rides the SAME generic LIVE/RECORD/REPLAY runner as every other adapter: one shared normalizer produces `FxRateEvidence` in all three modes, RECORD persists a sanitized raw payload under a deterministic recording key (provider|operation|pair+date), REPLAY replays it with byte-identical normalized output. Evidence carries the PROVIDER's own reference date (`observedAt` = midnight UTC of the fixing date, never the fetch instant) and CONNECTED authority. Resolution is LAYERED (`src/app/fxResolver.ts`): organisation budget/planning FX stays first-class and is what future-dated travel uses; the union of budget + external evidence feeds the unchanged deterministic selector (freshest effective observation wins; higher fact-ladder authority wins ties), so no source-name branching exists anywhere and a future spot rate can never be fabricated — evidence observed after the comparison instant is skipped, expired evidence fails closed. External outages contribute nothing rather than poisoning good budget evidence; with neither layer producing usable evidence, authority BLOCKs exactly as before. No engine change: `fx.ts`, authority, executor ceilings, intent freezing are untouched. Config: optional `providers.frankfurter.baseUrl` / `FRANKFURTER_BASE_URL`. Permanent tests: `test/northstar-frankfurter-fx.test.ts` (normalization, RECORD→REPLAY parity incl. downstream conversion equality, arbitrary ZAR/NOK pair, future-dated budget-FX precedence, empty-layer fail-closed BLOCK, invalid-row drop).

## ADR-053 — Authorised restoration may resolve a case without travel execution
**Status:** Accepted (NORTHSTAR final hero lifecycle/state)

A later authorised non-execution mutation (typically a programme commit that re-times a commitment) can make an already-open recovery case's trip viable again. The operator must not be asked to Resolve/Begin/Execute a recovery that is no longer needed, and Overview must project that traveller as Confirmed.

- **Legal transitions:** `ASSESSING`, `PLANNING`, `READY_TO_EXECUTE`, `AWAITING_TRAVELLER`, and `AWAITING_APPROVAL` may move directly to `RESOLVED` when `CaseVerifier` returns a resolution. `EXECUTING` still observes via `VERIFYING` first. `RESOLVED` remains terminal.
- **Reconcile prior open cases only:** `reconcilePriorOpenCasesIfTripViable` closes already-open cases on that trip when verification says `RESOLVED`. A brand-new case on a previously healthy trip is still opened (commitment fan-out / Case C). No traveller, event, or route names in this path.
- **CTA projection is server state:** workspace phase (`resolved | executing | execute | awaiting_authority | options | impacted`) is derived from the case/read-model, not `sessionStorage`. Approval is not recovery: an `APPROVED` intent that is not yet observed exposes Execute. Decline never executes.
- **Overview:** resolved travellers keep a `historyCaseId` so reopen shows the closed case; `activeCaseId` is reserved for open work.

Permanent tests: `test/case-lifecycle-state.test.ts`, `test/final-demo-lifecycle-convergence.test.ts`, `test/e2e/hero-lifecycle-rehearsal.test.ts`.
