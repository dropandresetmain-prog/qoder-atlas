...
> **R2 clarification (2026-09-19, see
> `docs/work/R2_CASE_DECISION_SURFACE_CONTRACT.md` §3/§9):** `RECOVERED` remains a
> valid NODE-level `LdgSemanticState`. At the GRAPH level the focused Case Graph has
> only three effective visual states — HEALTHY, CHECKING, DISRUPTED/SETTLED — and a
> fully recovered case reads as the new HEALTHY current world. There is NO separate
> RECOVERED graph architecture. CHECKING is the presentation of the existing
> `evaluation: PENDING_REASSESSMENT` lifecycle dimension (§FIG-7), never a new
> semantic verdict and never a rewrite of `semanticState`.
...
| FIG-5b — **Resolved for the focused Case surface by R2** (2026-09-19, see `docs/work/R2_CASE_DECISION_SURFACE_CONTRACT.md` §6.1) | R2 adds an optional `focusedGraph` block to `RecoveryCaseView` carrying backend-computed `causalNodeRefs`/`causalEdgeIds` (the ordered authoritative `causalPath` intersected with the produced `ldg` visible refs/edge ids) plus an explicit `unmappedCausalSteps` honest-gap list. The focused Case surface populates `causal` focus from this backend-supplied path, never adjacency, exactly as this row required. Other scopes (dashboard/incident) still must not populate `causal` without their own backend path. | Focused causal emphasis |
