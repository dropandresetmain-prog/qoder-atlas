...
| M9 — application integration | **C4 candidate (generic complete)** on `milestone-m9-product-integration`. CK1+CK2 generic Sarah loop, product surfaces, PG assemblers, `/api/v2`, Jordan multi-action/multi-stay landed. Fixture-lane IDs + Fable polish + Jordan Atlas/Nuitée E2E still external. Evidence: `docs/refactor/evidence/M9.md`. | Independent C4 review; then M10. |
