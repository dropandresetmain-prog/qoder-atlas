...
| M8 — authority/durable execution | **M8 candidate** on `milestone-m8-authority-execution` (Checkpoint 0 closed; authority/budget/execution/observation landed). C3 not claimed — awaits M7/M8 integration. | Scoped approvals, financial commitments, attempts/reconciliation and internal programme execution. Pre-M8 conditions closed: I-10 exact FX; traveller-payer UNKNOWN blocking; decision-time `currentAssessmentView`; reassessment `complete()` bounded retry/requeue. |
