...
| M8 — authority/durable execution | **M8 candidate**, integrated onto `integration/m7-m8-c3` (was `milestone-m8-authority-execution`; Checkpoint 0 closed; authority/budget/execution/observation landed) | Scoped approvals, financial commitments, attempts/reconciliation and internal programme execution. Pre-M8 conditions closed: I-10 exact FX; traveller-payer UNKNOWN blocking; decision-time `currentAssessmentView`; reassessment `complete()` bounded retry/requeue. |
| M7/M8 integration | **In progress** — C3 not claimed. Schema/seam reconciliation tracked in `docs/work/M7_M8_INTEGRATION_ACTIVE_TASK.md`; evidence lands in `docs/refactor/evidence/M7_M8_INTEGRATION.md` once gates are green. | Single coherent pipeline: M6 world → M7 strategy/plan → M8 authority/execution → observation/reassessment. |
