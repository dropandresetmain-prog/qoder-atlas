...
| M9 — application integration | Checkpoint 1 complete on `milestone-m9-product-integration` (target PG composition, strategy persistence, R-10 grants, IN-1, resolution gate, read models/LDG). Second scenario = Jordan S2 (CK1 contracts compatible; additive multi-action fields deferred to CK2). OpenRouter provider-neutral lane (`2eeff2a`) merged early. Checkpoint 2 waiting on Sarah/product handoff. Evidence: `docs/refactor/evidence/M9.md`. | Canonical runtime composition, APIs/read models and operator/traveller surfaces. |
