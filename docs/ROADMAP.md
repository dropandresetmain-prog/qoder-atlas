...
| M9 — application integration | Checkpoint 2 in progress on `milestone-m9-product-integration`. CK1 closed. Additive multi-action RMs, 150-min readiness rule, five-person cohort helper, bilateral programme-swap preview, wholeTripRecoveryPlan multi-intent fix landed. Sarah/Jordan fixture IDs still pending fixture lane. OpenRouter already merged. Evidence: `docs/refactor/evidence/M9.md`. | Canonical runtime composition, APIs/read models and operator/traveller surfaces. |
