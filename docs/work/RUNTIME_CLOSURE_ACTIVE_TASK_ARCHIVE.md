# Archived active-task ledger — T2 through runtime closure / B1

Archived on 2026-09-18 after B1 closed at
`82ae9b80f62a26d8b7e8e6277aa5bf6183ff44f0`.

This file preserves the long-running T2/R0/T3/B1 working memory exactly for provenance.
It is **not** the current task ledger. Current work lives in
...
| 11. Founder T2 latency: drain reassessment work in one wake | `c3faa28` (pushed) | Pre-fix production worker claimed **one** unit per 2s idle poll (~134s sleep for 67 units) while each assessment took ~344ms. Measured enqueue: 67 JOURNEY `INPUT_CHANGED` units via reverse lookup (`AGGREGATE\|RESERVATION` and coalesced `SCOPE\|INFORMATION_TOPIC:m6:unregistered-topic`), not a WORKSPACE generation fallback. Invalidation scope left broad (capture records those inputs). Fix: generic `drainAvailable` — claim a bounded batch, run pipelines concurrently, **complete/fail serially** (SERIALIZABLE), immediately take the next batch until empty or budget. Idle cadence stays 2s when the queue is empty. After: T2 drain **67 COMPLETED / EMPTY / ~16–17s**. 1–3s is not safely achievable without narrowing capture/invalidation (Investigate Now). Focused: m6Reassessment 15/15; T2 12/12; m9 E2E 1/1; typecheck/lint clean. Broad CURRENT PG gate **510/510** after truncating leftover inbox/outbox residue (same known test-DB hygiene issue as checkpoint 7, not a product defect). |
...
3. Confirm last settled counts remain visible; quiet “Reconciling changes…” may appear. Counts must not collapse to 0 or rebuild 0→49.
4. Settled result is 49 / 3 / 15 within roughly tens of seconds (backend drain ~17s plus up to ~2s poll), then stays visually still under continued polling.
...
| FT2-LATENCY | Founder physical test: “Reconciling changes…” lasts far too long. Measured: 67 JOURNEY units enqueued (reservation reverse lookup + coalesced unregistered-topic scope); production worker processed 1 item / 2s wake (~134s idle). | **Act Now — fixed** (checkpoint 11): generic in-wake drain (bounded batch, serial complete). After: ~17s trigger→queue empty. Remaining ~17s is capture/evaluate of 67 units, not idle sleep. **Investigate Now:** whether world capture should record unrelated reservations / `m6:unregistered-topic` as assessment inputs (narrowing would be a material capture/currentness change — not improvised here). Parked: SSE/WebSocket; progressive observability UI. |
