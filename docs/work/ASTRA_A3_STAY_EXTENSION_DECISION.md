...
## Original destination encounter required by stay replacement

WHAT WE KNOW: the provisioned journey has a destination STAY but zero intended visits. The existing replacement contract correctly requires a same-Journey landside visit, its scoped passport selection and complete entry evidence. WHAT WE DO NOT KNOW: the final live source/provider evidence and selected outcomes. KEY ASSUMPTION: an explicitly declared original destination visit belongs in source intent, rather than being invented by the planner. WHAT SHOULD BE TESTED NEXT: source materialization/idempotency, scoped credential/evidence publication, then the complete live composite recommendation.

Act Now: add only the explicit source visit needed by the existing destination stay and reuse that encounter for its replacement subinterval. Its jurisdiction, original window and trip purpose come from source facts. The proposed Japan overnight remains a separate encounter with independent evidence. Missing evidence is UNKNOWN, never an inferred exemption. Park automatic stay-to-visit inference: a hotel reservation alone does not establish cross-border intent.
