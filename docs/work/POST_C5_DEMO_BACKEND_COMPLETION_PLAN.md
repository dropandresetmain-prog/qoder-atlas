...
> **DELIVERY STRUCTURE SUPERSEDED.** Imported verbatim from
> `plan/post-c5-demo-backend-completion@22d63bc` during post-C5 convergence and kept
> for its technical analysis and provenance only.
>
> Its **six-package (WP1-WP6) delivery framing is no longer the execution structure.**
> The authoritative sequence is `IMPLEMENTATION_PLAN.md` §22 — Slice A, Founder Test A,
> Slice B, Founder Test B, submission rehearsal / M11 activation. Read WP1-WP6 below as
> a requirements catalogue, and §22 for what is actually built in what order.
>
> Two of its items are already closed and must not be re-planned:
> WP1 (projection freshness/revision) and WP3 (stable scoped graph identity) landed with
> the accepted live read-model lane `cbe5f83`; `M2-ACCESS-PATH-PLANNER` is resolved
> (see §22 and `docs/TESTING.md`).
