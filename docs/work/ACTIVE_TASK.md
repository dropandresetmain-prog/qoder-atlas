...
| 10. Founder T2 UX: hold SETTLED presentation during RECONCILING; skip no-op settled redraws | *(this commit)* | Root causes: (1) non-CURRENT assessments collapse population status to UNKNOWN → Confirmed 50→0→rebuild while M6 worker drains PENDING_REASSESSMENT; (2) polling always swapped `outerHTML` every ~2s even when only `generatedAt` moved. Fix: overview publishes `populationAssessmentLifecycle` from authoritative PENDING_REASSESSMENT counts; HTML embeds lifecycle + `data-stable-revision`; polling holds last SETTLED DOM while RECONCILING (shows quiet “Reconciling changes…”), applies only on SETTLED revision change. No FE readiness inference. Focused: overviewPopulationLifecycle 4/4; overviewPollingSwap 9/9; m9-product-surfaces 8/8 |
