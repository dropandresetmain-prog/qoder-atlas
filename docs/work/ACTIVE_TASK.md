...
| 7. Final Phase-B candidate: broad CURRENT PG gate + F8/Nadia dispositions in ledger | (this commit) | Full postgres suite 504/504 pass (`node scripts/run-suite.mjs postgres`, 270s) including t2F1FailureInjection 7/7, t2F5ServiceIdentity 1/1, t2F3HttpValidation 5/5, t2F7SelectionGuard 3/3, t2ProviderDisruptionReprotection 12/12; F8 Park disposition documented; typecheck clean; branch tip = origin verified after push. Note: a prior gate attempt stalled on ~97k residue PENDING outbox rows in the shared test DB (test drain-loop in inboxOutbox is linear in residue); truncated test-residue queue tables (outbox, inbox_deliveries, inbox_delivery_conflicts, inbox_work) and re-ran — not a product defect |
